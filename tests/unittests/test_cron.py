...
class CronTestCase(TestCase):
    def test_parse_crontab(self):
        self.assertEqual(
            ...
            len(
                cron._parse_crontab(
                    f"{Utils._get_path_name()}{os.sep}resources{os.sep}crontab"
                )
            ),
        )
    ...
    def test_parse_crontab_no_valid_file(self):
        with self.assertRaises(SystemExit):
            cron._parse_crontab(
                f"{Utils._get_path_name()}{os.sep}resources{os.sep}crontab1"
            )

    def test_parse_crontab_crontab_no_valid_lines(self):
        with self.assertRaises(SystemExit):
            cron._parse_crontab(
                f"{Utils._get_path_name()}{os.sep}resources{os.sep}crontab_error"
            )
    ...
    def test_execute_command_valid_command(self, subprocess_run_mock):
        ...
        self.assertEqual(
            [
                "INFO:exec:Executing command ls -la",
                "INFO:exec:Standard output: test",
                "INFO:exec:Standard error: ",
            ],
            cm.output,
        )
    ...
    def test_execute_command_non_valid_command(self, subprocess_run_mock):
        ...
        self.assertEqual(
            [
                "INFO:exec:Executing command test",
                "INFO:exec:Standard output: test",
                "INFO:exec:Standard error: test",
            ],
            cm.output,
        )
