...
_attribute = ["minute", "hour", "day", "month", "isoweekday", "year"]
_alternate = {
    3: {
        "jan": 1,
        "feb": 2,
...
class _Matcher(object):
    ...
    @staticmethod
    def _parse_crontab(which, entry):
        """
        ...
        if entry in ("*", "?"):
            if entry == "?":
                _assert(
                    which in (2, 4), "cannot use '?' in the %r field", _attribute[which]
                )
            return None, _end
        ...
        # for the last 'friday' of the month, for example
            ...
            _assert(
                which == 4, "you can only specify a leading 'L' in the 'weekday' field"
            )
...
class CronTab(object):
    ...
    def _make_matchers(self, crontab):
        ...
        matchers = [
            _Matcher(which, entry) for which, entry in enumerate(crontab.split())
        ]
