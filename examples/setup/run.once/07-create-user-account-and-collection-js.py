...
dictionary = {
    ...
    "roles": [
        {"role": "readWrite", "db": f"{utils.get_env_variable('MONGO_INITDB_NAME')}"}
    ],
}
o = open(
    f"{utils.get_env_variable('IMAGE_CONFIG_DIR')}{os.sep}create-db-test-data.js", "r"
)

f = open(
    f"{utils.get_env_variable('STORAGE_CONF_DIR')}{os.sep}create-user-account.js", "w"
)
